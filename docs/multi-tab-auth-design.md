...
- 检查：有认证流程在进行
- 第 2-N 个请求：返回等待页面（建立 SSE 连接）
- 第 N+1 个请求：重定向到 Logto（让它去登录）
...
1. **app.com** → 返回等待页面，建立 SSE 连接
2. **bpp.com** → 返回等待页面，建立 SSE 连接
3. **cpp.com** → 重定向到 Logto
4. 用户在 cpp.com 登录 → auth 创建 Session → 广播 SSE 事件
5. app.com 和 bpp.com 收到事件 → 自动跳转回原页面
